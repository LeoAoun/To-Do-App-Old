...
function getTodos(
  ...
  axios.get(`http://localhost:3333/todo/user/${userId}`).then((response) => {
...
function createTodo(
  ...
  axios
    .post("http://localhost:3333/todo", todoData, {
      headers: {
...
function handleCheck(
  ...
  axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;
  axios
    .put(`http://localhost:3333/todo/${todo.id}`, {
      title: todo.title,
...
function deleteTodo(
  ...
  axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;
  axios
    .delete(`http://localhost:3333/todo/delete/${todo.id}`)
    .then((response) => {
...
export { getTodos, createTodo, handleCheck, deleteTodo };
